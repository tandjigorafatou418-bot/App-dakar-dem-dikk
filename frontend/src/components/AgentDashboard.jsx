...
<<<<<<< HEAD
  ...
  Ticket
=======
  Scan, Plus, CheckCircle, XCircle, AlertTriangle, Users, Bus, CreditCard,
  Clock, MapPin, Phone, QrCode, Download, Search, Filter, Ticket, Camera
...
=======
    departure: '', destination: '', date: '', time: '', passengers: 1,
    passengerName: '', phone: '', paymentMethod: ''
  });

  const [incidentForm, setIncidentForm] = useState({
    type: '', description: '', busNumber: '', priority: 'normal'
  });

  const todayStats = {
    ticketsSold: 45, revenue: 67500, validatedTickets: 38, incidents: 2
  };

  const recentTickets = [
    { id: 'DDK1704123456', route: 'Dakar → Pikine', passenger: 'Absa Niang', time: '14:30', price: 1000, status: 'Vendu', paymentMethod: 'Wave' },
    { id: 'DDK1704123457', route: 'Plateau → Guédiawaye', passenger: 'Arame Cisse', time: '15:15', price: 600, status: 'Validé', paymentMethod: 'Orange Money' },
    { id: 'DDK1704123458', route: 'Dakar → Thiès', passenger: 'Noguay Diouf', time: '07:00', price: 7500, status: 'Vendu', paymentMethod: 'Espèces' }
  ];

  const calculatePrice = (departure, destination) => {
    const prices = {
      'Dakar-Thiès': 2500, 'Dakar-Kaolack': 4500, 'Dakar-Saint-Louis': 5500,
      'Plateau-Colobane': 500, 'Colobane-Pikine': 750, 'Dakar-Pikine': 500, 'Plateau-Guediawaye': 600
    };
...
=======
    alert(`✅ Billet vendu!\n\nNuméro: ${newTicket.id}\nPrix: ${newTicket.price} FCFA`);
    setTicketSaleForm({ departure: '', destination: '', date: '', time: '', passengers: 1, passengerName: '', phone: '', paymentMethod: '' });
>>>>>>> 48f4252 (Mise à jour finale du projet)
  };
...
=======
      setScanResult({
        ticketId: 'DDK1704123456', passenger: 'Absa Niang', route: 'Dakar → Thiès',
        date: '2025-01-15', time: '14:30', status: Math.random() > 0.3 ? 'valid' : 'invalid', seatNumber: 'A12'
      });
>>>>>>> 48f4252 (Mise à jour finale du projet)
      setIsScanning(false);
    }, 2000);
...
=======
    ...
    setIncidentForm({ type: '', description: '', busNumber: '', priority: 'normal' });
>>>>>>> 48f4252 (Mise à jour finale du projet)
  };
...
=======
      ...
      {activeSection === 'ticket-sale' && (
        <div className="bg-white rounded-lg shadow-md p-6">
          <h2 className="text-2xl font-bold mb-6 flex items-center">
            <Ticket className="w-6 h-6 mr-2 text-blue-600" />Vente de Billets
          </h2>
          <form onSubmit={handleTicketSale} className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              {['departure', 'destination'].map((field, i) => (
                <div key={field}>
                  ...
                  <select value={ticketSaleForm[field]} required
                    onChange={(e) => setTicketSaleForm({ ...ticketSaleForm, [field]: e.target.value })}
                    className="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500">
                    <option value="">Sélectionner...</option>
                    {(i === 0 ? ['Dakar', 'Plateau', 'Pikine', 'Thiès'] : ['Pikine', 'Guédiawaye', 'Thiès', 'Kaolack']).map(o => 
            ...
            <div className="grid grid-cols-3 gap-4">
              <div>
                ...
                <input type="date" value={ticketSaleForm.date} required min={new Date().toISOString().split('T')[0]}
                  onChange={(e) => setTicketSaleForm({ ...ticketSaleForm, date: e.target.value })}
                  className="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500" />
              </div>
              <div>
                <label className="block text-sm font-medium mb-2">Heure</label>
                <select value={ticketSaleForm.time} required
                  onChange={(e) => setTicketSaleForm({ ...ticketSaleForm, time: e.target.value })}
                  className="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500">
                  <option value="">Sélectionner...</option>
                  {Array.from({length: 13}, (_, i) => `${6+i}:00`).map(t => <option key={t} value={t}>{t}</option>)}
                </select>
              </div>
              <div>
                <label className="block text-sm font-medium mb-2">Places</label>
                <input type="number" min="1" max="10" value={ticketSaleForm.passengers} required
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input type="text" value={ticketSaleForm.passengerName} required
                  onChange={(e) => setTicketSaleForm({ ...ticketSaleForm, passengerName: e.target.value })}
                  className="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500" placeholder="Nom complet" />
              </div>
              <div>
                <label className="block text-sm font-medium mb-2">Téléphone</label>
                <input type="tel" value={ticketSaleForm.phone} required
                  onChange={(e) => setTicketSaleForm({ ...ticketSaleForm, phone: e.target.value })}
                  className="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500" placeholder="+221 XX XXX XX XX" />
              </div>
            </div>
            <div>
              ...
              <div className="grid grid-cols-3 gap-4">
                {['Wave', 'Orange Money', 'Espèces'].map(m => (
                  <button key={m} type="button"
                    ...
                    className={`p-3 border-2 rounded-lg ${ticketSaleForm.paymentMethod === m ? 'border-blue-500 bg-blue-50' : 'border-gray-300'}`}>
                    {m}
                  </button>
                ))}
              </div>
      ...
      {activeSection === 'qr-scanner' && (
        <div className="bg-white rounded-lg shadow-md p-6">
          <h2 className="text-2xl font-bold mb-6 flex items-center">
            <QrCode className="w-6 h-6 mr-2 text-green-600" />Scanner QR Code
          </h2>
          <div className="text-center">
            {!isScanning && !scanResult && (
              <div>
                ...
                <button onClick={handleQRScan} className="bg-green-600 hover:bg-green-700 text-white px-8 py-3 rounded-lg font-semibold">
                  Démarrer le scan
                </button>
              </div>
            )}
            ...
            {scanResult && (
              <div className={`p-6 rounded-lg ${scanResult.status === 'valid' ? 'bg-green-50 border-2 border-green-500' : 'bg-red-50 border-2 border-red-500'}`}>
                {scanResult.status === 'valid' ? <CheckCircle className="w-16 h-16 text-green-600 mx-auto mb-4" /> : <XCircle className="w-16 h-16 text-red-600 mx-auto mb-4" />}
                <h3 className={`text-2xl font-bold mb-4 ${scanResult.status === 'valid' ? 'text-green-800' : 'text-red-800'}`}>
                ...
                <button onClick={() => setScanResult(null)} className="mt-6 bg-gray-600 hover:bg-gray-700 text-white px-6 py-2 rounded-lg">
                  Nouveau scan
                </button>
              </div>
            )}
      ...
      {activeSection === 'incidents' && (
        <div className="space-y-6">
          <div className="bg-white rounded-lg shadow-md p-6">
            <h2 className="text-2xl font-bold mb-6 flex items-center">
              <AlertTriangle className="w-6 h-6 mr-2 text-orange-600" />Signaler un Incident
            </h2>
            <form onSubmit={handleIncidentReport} className="space-y-4">
              <div>
                ...
                <select value={incidentForm.type} required
              ...
              <div>
                ...
                <input type="text" value={incidentForm.busNumber} required
                  onChange={(e) => setIncidentForm({ ...incidentForm, busNumber: e.target.value })}
                  className="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-orange-500" placeholder="Ex: DDD-001" />
              </div>
              <div>
                <label className="block text-sm font-medium mb-2">Priorité</label>
                <div className="grid grid-cols-3 gap-4">
                  {[{v:'low',l:'Faible',c:'green'},{v:'normal',l:'Normal',c:'blue'},{v:'high',l:'Urgent',c:'red'}].map(p => (
                    <button key={p.v} type="button"
                      onClick={() => setIncidentForm({ ...incidentForm, priority: p.v })}
                      className={`p-3 border-2 rounded-lg ${incidentForm.priority === p.v ? `border-${p.c}-500 bg-${p.c}-50` : 'border-gray-300'}`}>
                      {p.l}
                    </button>
                  ))}
                </div>
              ...
              <div>
                ...
                <textarea value={incidentForm.description} required rows={4}
                  onChange={(e) => setIncidentForm({ ...incidentForm, description: e.target.value })}
                  className="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-orange-500" placeholder="Décrivez l'incident..." />
              </div>
              <button type="submit" className="w-full bg-orange-600 hover:bg-orange-700 text-white py-3 rounded-lg font-semibold">
                Signaler l'incident
              </button>
            </form>
          </div>
          <div className="bg-white rounded-lg shadow-sm p-6 border">
            ...
            <div className="space-y-3">
              {[{type:'Retard',bus:'DDD-005',time:'30 min',priority:'normal'},{type:'Panne',bus:'DDD-012',time:'2h',priority:'high'}].map((inc,i) => (
                <div key={i} className="p-4 border rounded-lg">
                  <div className="flex justify-between">
                    ...
                    <span className={`px-2 py-1 rounded text-xs font-medium ${inc.priority === 'high' ? 'bg-red-100 text-red-800' : 'bg-blue-100 text-blue-800'}`}>
      ...
      <QRCodeModal isOpen={showQRModal} onClose={() => setShowQRModal(false)} ticket={selectedTicket} />
    </div>
  );
}
>>>>>>> 48f4252 (Mise à jour finale du projet)
